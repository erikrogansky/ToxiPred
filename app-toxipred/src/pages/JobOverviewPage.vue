...
async function downloadPdf() {
  ...
  try {
    ...
    const drawHeader = (logoImg: string | null) => {
      const headerY = margin;
      
      if (logoImg) {
        doc.addImage(logoImg, 'PNG', margin, headerY, 38, 7.6);
      } else {
        drawRoundedRect(margin, headerY, 38, 10, 2, lightGray);
        doc.setDrawColor(...primaryGreen);
        doc.setLineWidth(0.6);
        doc.circle(margin + 6, headerY + 5, 3.5);
        doc.setLineWidth(0.5);
        doc.line(margin + 4, headerY + 5, margin + 5.5, headerY + 6.5);
        doc.line(margin + 5.5, headerY + 6.5, margin + 8, headerY + 3.5);
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(...darkText);
        doc.text('ToxiPred', margin + 12, headerY + 6.5);
      }
      
      doc.setFontSize(8);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...grayText);
      doc.text(`Report ID: ${jobId}`, pageWidth - margin, headerY + 3, { align: 'right' });
      doc.text(`Generated: ${currentDate}`, pageWidth - margin, headerY + 7, { align: 'right' });
    };
    ...
    const drawFooter = () => {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...grayText);
      doc.text(`Generated by ToxiPred • ${currentDate}`, pageWidth / 2, footerY + 6, { align: 'center' });
      ...
      doc.text(disclaimerLines, pageWidth / 2, footerY + 11, { align: 'center' });
    };
    
    // Helper to check and add new page
    const checkNewPage = (neededHeight: number, logoImg: string | null) => {
    ...
    // Create logo and draw first page header/footer
    ...
    checkNewPage(contentHeight + 20, logoImg);
    ...
    if (smiles.value) {
      ...
      checkNewPage(structureHeight + 20, logoImg);
    ...
    if (featureNames.value.length > 0 && featureScores.value.length > 0) {
      ...
      if (chartImg) {
        // Maintain aspect ratio (600:350 from original) but use 80% of content width
        const chartAspectRatio = 600 / 350;
        const imgWidth = contentWidth * 0.8;
        const imgHeight = imgWidth / chartAspectRatio;
        const imgX = margin + (contentWidth - imgWidth) / 2; // Center the chart
        
        drawRoundedRect(imgX - 2, y - 2, imgWidth + 4, imgHeight + 4, 3, white, [220, 220, 220]);
        doc.addImage(chartImg, 'PNG', imgX, y, imgWidth, imgHeight);
        y += imgHeight + 10;
      }
    }
    ...
    if (featureNames.value.length > 0) {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...darkText);
      doc.text('Descriptor', tableX + 2, y + 5);
      doc.text('Type', tableX + colWidths.desc + 2, y + 5);
      doc.text('Value', tableX + colWidths.desc + colWidths.type + 2, y + 5);
      doc.text('Importance', tableX + colWidths.desc + colWidths.type + colWidths.value + 2, y + 5);
      ...
      for (const desc of descriptorData) {
        ...
        doc.setFontSize(10);
        doc.setTextColor(...darkText);
        doc.text(desc.displayName, tableX + 2, y + 5.15);
        ...
        drawRoundedRect(tableX + colWidths.desc, y + 1.5, typeBadgeWidth, 5, 2.5, badgeColors.bg);
        doc.setTextColor(...badgeColors.text);
        doc.text(typeText, tableX + colWidths.desc + 2.5, y + 5);
        
        // Value
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(...darkText);
        doc.text(desc.value, tableX + colWidths.desc + colWidths.type + 2, y + 5.15);
